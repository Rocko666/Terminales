...
def tmp_fact_mov_final_upd_csts(VtablaTmpFactMov):
    qry="""
    SELECT fecha_factura,
    bill_status,
    sri_authorization_date,
    ...
    plan_nombre,
    tarifa_basica
    FROM {VtablaTmpFactMov}
    """.format(VtablaTmpFactMov=VtablaTmpFactMov)
    print(qry)
    return qry
...
def tmp_concepto_articulo_csts(VtablaTmpCostFactFin):
    qry="""
...
def tmp_fact_final_csts(VtablaTmpCostFactFin):
    qry="""
    SELECT a.fecha_factura,
    a.bill_status,
    a.sri_authorization_date,
...
def tmp_costo_fact_final_v3_csts(vTablaCostFinV2):
    qry="""
...
def sql_file(val_fecha_formato,val_dia_uno,val_fecha_ini):
    qry="""
    SELECT a.fecha_proceso AS fecha_proceso
    ...
    ,a.oficina AS oficina,a.account_num AS account_num
    ...
    (CASE WHEN fecha_factura IS NULL THEN '01/01/1990' ELSE concat_ws('/',SUBSTR(fecha_factura,9,2),SUBSTR(fecha_factura,6,2),SUBSTR(fecha_factura,1,4)) END) AS fecha_factura,
    linea_negocio,
    segmento,
    sub_segmento,
    segmento_final,
    ...
    num_factura,
    num_factura_relacionada,
    (CASE WHEN fecha_factura_relacionada IS NULL THEN '01/01/1990' ELSE  concat_ws('/',SUBSTR(fecha_factura_relacionada,9,2),SUBSTR(fecha_factura_relacionada,6,2),SUBSTR(fecha_factura_relacionada,1,4)) END) AS fecha_factura_relacionada,
    oficina,
    account_num,
    nombre_cliente,
    ...
    num_abonado,
    movimiento,
    (CASE WHEN id_tipo_movimiento IS NULL THEN '-1' ELSE id_tipo_movimiento END) AS id_tipo_movimiento,
    (CASE WHEN id_producto IS NULL THEN '-1' ELSE id_producto END) AS id_producto,
    plan_codigo,
    plan_nombre,
    (CASE WHEN tarifa_basica IS NULL THEN '' ELSE CAST(CAST(tarifa_basica AS decimal(12,2)) AS string) END) AS tarifa_basica,
    usuario_final,
    nombre_usuario_final,
    tipo_venta,
    cuotas_financiadas,
    ...
    FROM db_desarrollo2021.otc_t_terminales_simcards_prycldr WHERE p_fecha_factura>={val_fecha_ini} AND p_fecha_factura<{val_dia_uno}
    ...
    clasificacion_terminal,
    cantidad,
    CAST(CAST(monto AS decimal(12,2)) AS string) AS monto,num_abonado,movimiento,
    (CASE WHEN id_tipo_movimiento IS NULL THEN '-1' ELSE id_tipo_movimiento END) AS id_tipo_movimiento,
    (CASE WHEN id_producto IS NULL THEN '-1' ELSE id_producto END) AS id_producto,
    plan_codigo,
    plan_nombre,
    ...
    (CASE WHEN cuota_inicial IS NULL THEN '-1' ELSE CAST(CAST(cuota_inicial AS decimal(12,2)) AS string) END) AS cuota_inicial,
    canal_comercial,
    (CASE WHEN id_canal IS NULL THEN '-1' ELSE id_canal END) AS id_canal,
    oficina_usuario AS nom_distribuidor,
    ruc_distribuidor,
    codigo_plaza,
    nom_plaza,ciudad,
    ...
    nuevo_subcanal,
    (CASE WHEN id_sub_canal IS NULL THEN '-1' ELSE id_sub_canal END) AS id_sub_canal,tipo_movimiento_mes,
    (CASE WHEN fecha_alta IS NULL THEN '01/01/1990' ELSE concat_ws('/',SUBSTR(date_format(fecha_alta,'yyyy-MM-dd'),9,2),SUBSTR(date_format(fecha_alta,'yyyy-MM-dd'),6,2),SUBSTR(date_format(fecha_alta,'yyyy-MM-dd'),1,4)) END) AS fecha_alta,
    (CASE WHEN CAST(CAST(antiguedad_meses AS decimal(12,2)) AS string)<0 THEN '-1' ELSE CAST(CAST(antiguedad_meses AS decimal(12,2)) AS string) END) AS antiguedad_meses,
    linea_negocio_homologado,
    id_hash
    FROM db_desarrollo2021.otc_t_terminales_simcards_prycldr WHERE p_fecha_factura>={val_fecha_ini} AND p_fecha_factura<{val_dia_uno}
    AND clasificacion IN ('ACCESORIOS','TERMINALES') AND tipo_cargo = 'CARGO' AND tipo_documento='NOTA DE CREDITO' 
    ...
    AND concat_ws('',SUBSTR(fecha_factura_relacionada,1,4),SUBSTR(fecha_factura_relacionada,6,2),SUBSTR(fecha_factura_relacionada,9,2))<'{val_dia_uno}') a
    """.format(val_fecha_formato=val_fecha_formato,val_dia_uno=val_dia_uno,val_fecha_ini=val_fecha_ini)
    print(qry)
    return qry
